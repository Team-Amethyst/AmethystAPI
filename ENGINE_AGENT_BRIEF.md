...
**Developer portal (human operators, not Draft):** The Engine serves `public/index.html`. **`GET /api/keys/status`** and **`POST /api/keys/issue`** mint MongoDB keys by default (plaintext returned once); set **`KEY_ISSUANCE_ENABLED=0|false|off`** to disable. Optional **`KEY_ISSUANCE_SECRET`** + header **`X-Key-Issuance-Token`**. Draft continues to use only **`x-api-key`** on Brain routes.
...
**News signals realtime:** ETag / `304`, Redis fast path, per–API-key webhook registration (`PATCH /api/account/keys/:id/news-signals-webhook`), optional global env URL — see **`engine-contract/NEWS_SIGNALS_REALTIME_ENGINE_PROMPT.md`**. Draft’s mirror doc: **`apps/api/engine-contract/NEWS_SIGNALS_REALTIME_ENGINE_PROMPT.md`** (AmethystDraft).
